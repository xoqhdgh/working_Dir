...
void make_hist(){
	...
	//TString dirname = "wjets_500Mjj_4mom6000toinf";
	TString dirname = "signal_6000mass";

        TChain* inChain = new TChain("LHEF");
        inChain->Add("/home/taebh/workspace/MG5_aMC_v2_6_4/VBFWprime/gridpack/"+dirname+"/condorOut/*.root");
	//inChain->Add("/home/taebh/workspace/MG5_aMC_v2_6_4/VBFWprime/gridpack/"+dirname+"/Events/run_01/out.root");

        TClonesArray* particleTCA = new TClonesArray("TRootLHEFParticle");

	TFile* outfile = new TFile("Signal/"+dirname+"_VBFcut.root","recreate");
	...
	TH1F* Mjj = new TH1F("Mjj","Mjj",10000,0,10000);
	TH1F* Zeppenfeld = new TH1F("Zeppenfeld","Zeppenfeld",10000,0,20);
	TH1F* Centrality = new TH1F("Centrality","Centrality",10000,0,10);

        TH1F* delta_R_jj = new TH1F("delta_R_jj","delta_R_jj",10000,0,20);
        TH1F* ratio_jj = new TH1F("ratio_jj","ratio_jj",10000,0,20);
        TH1F* delta_R_ej1 = new TH1F("delta_R_ej1","delta_R_ej1",10000,0,20);
        TH1F* delta_R_ej2 = new TH1F("delta_R_ej2","delta_R_ej2",10000,0,20);
        TH1F* delta_R_wj1 = new TH1F("delta_R_wj1","delta_R_wj1",10000,0,20);
        TH1F* delta_R_wj2 = new TH1F("delta_R_wj2","delta_R_wj2",10000,0,20);

/****************************************************************************************************************************************/

	int totalN = inChain->GetEntries();
	int cnt_selected_event=0;
	...
	for(int eventLoop=0; eventLoop<totalN; eventLoop++){
		...
		/*if(ev_delta_Phi<=0.8)continue;
		if(ePt/vPt<=0.1||ePt/vPt>=3)continue;*/
		...
		Centrality->Fill(fabs((wEta-(j1Eta+j2Eta)/2)/(j1Eta-j2Eta)));
	...
	cout << "Total Event Number : " << totalN <<endl;
	cout << "Selected Event Number : " << cnt_selected_event << endl;
